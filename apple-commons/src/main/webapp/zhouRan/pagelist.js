...
function build_nav(result) {
    ...
    let str = "";
    if(pageinfo.hasPreviousPage === false){
        str += "<li class='disabled'><a>首页</a></li>";
        str += "<li class='disabled'>";
    }else {
        str += "<li onclick='toPage(1)'><a>首页</a></li>";
        str += "<li onclick='toPage("+pageinfo.prePage+")'>";
    }
    str += '<a aria-label="Previous"><span aria-hidden="true">&laquo;</span>';
    str += '</a></li>';

    $.each(pageinfo.navigatepageNums,function (index,obj) {
        if(obj === pageinfo.pageNum){
            str += `<li class="active"><a>${obj }</a></li>`;
        }else {
            str += `<li onclick="toPage(${obj })"><a>${obj }</a></li>`;
        }
    });
    if(pageinfo.hasNextPage === false){
        str += '<li class="disabled">';
    }else {
        str += '<li onclick="toPage('+pageinfo.nextPage+')">';
    }
    str += '<a aria-label="Next">';
    ...
    if(pageinfo.hasNextPage === false){
        ...
        str += '<li onclick="toPage('+pageinfo.pages+')">';
    }
    str += '<a>末页</a></li>';
